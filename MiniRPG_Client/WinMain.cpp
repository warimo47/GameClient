...
void ProcessKeyDown(WPARAM wParam)
{
	...
	else
	{
		...
		case 'A':
		{
			sendPacketType = CS_ATTACK;
		}
			break;
		case VK_UP:
		{
			dir = 0;
			sendPacketType = CS_MOVE;
		}
			break;
		case VK_RIGHT:
		{
		...
		if (canSendPacket)
		{
			switch (sendPacketType)
			{
			case CS_MOVE:
			{
				cs_move *my_packet = reinterpret_cast<cs_move *>(send_buffer);
				my_packet->size = sizeof(cs_move);
				my_packet->type = CS_MOVE;
				my_packet->dir = dir;
				send_wsabuf.len = sizeof(cs_move);
				DWORD iobyte;
			...
			case CS_ATTACK:
			{
				cs_attack *my_packet = reinterpret_cast<cs_attack *>(send_buffer);
				my_packet->size = sizeof(cs_attack);
				my_packet->type = CS_ATTACK;
				send_wsabuf.len = sizeof(cs_attack);
				DWORD iobyte;
...
void ProcessPacket(char* pBuf)
{
	...
	case SC_POSITION_INFO:
		g_cSceneManager->PosUpdate(reinterpret_cast<sc_position_info*>(pBuf));
		break;
	case SC_CHAT:
		g_cSceneManager->Chat(reinterpret_cast<sc_chat*>(pBuf));
		break;
	case SC_STAT_CHANGE:
		g_cSceneManager->StatChange(reinterpret_cast<sc_stat_change*>(pBuf));
		break;
	case SC_REMOVE_OBJECT:
		g_cSceneManager->RemoveObject(reinterpret_cast<sc_remove_object*>(pBuf));
		break;
	case SC_ADD_OBJECT:
		g_cSceneManager->AddObject(reinterpret_cast<sc_add_object*>(pBuf));
		break;
	default:
		break;
	}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SOCKET:
		...
		break;
	case WM_PAINT:
		if (g_cSceneManager == nullptr) break;
		{
			PAINTSTRUCT ps;
			HDC hdc = BeginPaint(hWnd, &ps);

			// Double buffering
			HBITMAP hViewBit = CreateCompatibleBitmap(hdc, g_cSceneManager->m_nWin_Width, g_cSceneManager->m_nWin_Height);
			...
			// Double buffering
			BitBlt(hdc, 0, 0, g_cSceneManager->m_nWin_Width, g_cSceneManager->m_nWin_Height, hViewdc, 0, 0, SRCCOPY);
